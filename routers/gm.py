...
async def disk_usage(x_relay_secret: str = Header(...)):
    """Disk footprint of the relay's own data (db + localised images) plus the
    hosting disk's used/free totals — Render instances have small disks, so
    ScenePlay shows this next to the relay connection status."""
    if not verify_gm_secret(x_relay_secret):
        raise HTTPException(status_code=401, detail="Invalid relay secret")
    import shutil
    ...
    uploads = {name: _tree_size(os.path.join(root, 'portal', name))
               for name in ('portraits', 'battlemaps', 'monsters')}
    du = shutil.disk_usage(root)
    return {
        'ok': True,
        'db_bytes':      db_bytes,
        'uploads':       uploads,
        'uploads_bytes': sum(uploads.values()),
        'data_bytes':    db_bytes + sum(uploads.values()),
        'disk_total':    du.total,
        'disk_used':     du.used,
        'disk_free':     du.free,
    }
